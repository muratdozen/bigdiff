...
def write_hashed_line_to_bucket_file(output_dirpath, num_buckets, bucket_number, line, output_file_handles):
    # bucket files' names are prepended with '0' so that all bucket filenames have the same length.
    # e.g if num_buckets is 457, and we're currently writing to bucket_number=9,
    # the bucket file will be numbered 009.
    bucket_number_str = '0' * (len(str(num_buckets - 1)) - len(str(bucket_number))) + str(bucket_number)
...
def get_bucket_filenames(dirpath):
    ...
    bucket1_filenames = [join_abs_path(bucket1_dirpath, f) for f in bucket1_filenames]
    bucket2_filenames = [join_abs_path(bucket2_dirpath, f) for f in bucket2_filenames]

    return bucket1_filenames, bucket2_filenames


def extract_original_lines_from_hashes_of_lines(hashes_of_lines, hash_func, filename):
    """
    Traverse all lines in filename to see if any lines' hash is contained in hashes_of_lines.
    """
    results = set()
...
def bucketize(hash_func, input_filename, num_buckets, output_dirpath):
    """
    Distribute each line in input_filename to bucket files according to the value of hash(line).
    """
    logging.info("Processing file %s", input_filename)
    # while bucketizing, it is preferred to keep a handle to all bucket files and never close them until the end,
    # instead of closing them after each write so that we can make use of gzip's compression power.
    output_file_handles = dict()
    with gzip.open(input_filename, 'r') as file:
        for line in file:
            line = line.strip()
            hashed_line = hash_func(line)
            bucket_number = compute_bucket_number_from_hashed_line(hashed_line, num_buckets)
            write_hashed_line_to_bucket_file(output_dirpath, num_buckets, bucket_number, str(hashed_line), output_file_handles)
...
def diff_all_buckets(dirpath):
    ...
    while i < len(bucket1_filenames) and j < len(bucket2_filenames):
        bucket1_filename, bucket2_filename = bucket1_filenames[i], bucket2_filenames[j]
        ...
        if bucket1_filename_basepath == bucket2_filename_basepath:
            diff_left, diff_right = diff_two_bucket_files(bucket1_filename, bucket2_filename)
            ...
            if bucket1_filename_basepath < bucket2_filename_basepath:
                bucket1_lines = read_gzip_file_lines_into_int_set(bucket1_filename)
                resulting_diff_left.update(bucket1_lines)
                i += 1
            else:
                bucket2_lines = read_gzip_file_lines_into_int_set(bucket2_filename)
...
def get_filenames_from_input_directory(dirpath):
    """
    The input directory should contain only two files; namely the file that are to be diffed.
    From these two files, the lexicographically smaller filename is treated as File1,
    and the other as File2.
    """
...
def create_temp_dirs(dirpath):
    """
    Creates a temporary directory inside the input directory and two other directories
    (named '1' and '2') inside the temporary directory.
    """
    buckets_dirpath = join_abs_path(dirpath, TEMP_DIR_NAME)
...
def write_diffs_to_output_files(dirpath, diff_left, diff_right):
    logging.info("Writing diffs to directory %s inside %s", OUTPUT_DIR_NAME, dirpath)
    output_dirpath = join_abs_path(dirpath, OUTPUT_DIR_NAME)
    os.mkdir(output_dirpath)
    diffleft_filename = join_abs_path(output_dirpath, OUTPUT_DIFF_LEFT_FILENAME)
    diffright_filename = join_abs_path(output_dirpath, OUTPUT_DIFF_RIGHT_FILENAME)
...
def bigdiff(dirpath, num_buckets, hash_func):
    ...
    try:
        ...
        buckets_dirpath, buckets1_dirpath, buckets2_dirpath = create_temp_dirs(dirpath)
        ...
        diff_left = extract_original_lines_from_hashes_of_lines(hashed_diff_left, hash_func, filename1)
        diff_right = extract_original_lines_from_hashes_of_lines(hashed_diff_right, hash_func, filename2)

        logging.info("Bigdiff obtained the diff successfully. Diff left: %s lines, diff right: %s lines", len(diff_left), len(diff_right))
        # save results to file system
        output__dirpath = write_diffs_to_output_files(dirpath, diff_left, diff_right)
